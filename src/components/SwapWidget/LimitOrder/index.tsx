...
import { CICZ, JSBI, Token, TokenAmount, Trade } from '@everestswap/sdk';
...
const LimitOrder: React.FC<Props> = ({ swapType, setSwapType, isLimitOrderVisible }) => {
  ...
  const getInputCurrency = () => {
    if (gelatoInputCurrency && gelatoInputCurrency?.symbol === CICZ[chainId].symbol) {
      return CICZ[chainId];
    } else if (inputTokenInfo && inputTokenInfo?.symbol === CICZ[chainId].symbol) {
      return CICZ[chainId];
  ...
  const getOutputCurrency = () => {
    if (gelatoOutputCurrency && gelatoOutputCurrency?.symbol === CICZ[chainId].symbol) {
      return CICZ[chainId];
    } else if (outputTokenInfo && outputTokenInfo?.symbol === CICZ[chainId].symbol) {
      return CICZ[chainId];
  ...
  const onCurrencySelect = useCallback(
    (currency) => {
      ...
      if (currency?.symbol === CICZ[chainId].symbol) {
