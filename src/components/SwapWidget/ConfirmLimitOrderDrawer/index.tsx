import { CICZ, Token, Trade, TradeType } from '@everestswap/sdk';
...
const ConfirmLimitOrderDrawer: React.FC<Props> = (props) => {
  ...
  const getInputCurrency = () => {
    if (inputCurrency1 && inputCurrency1?.symbol === CICZ[chainId].symbol) {
      return CICZ[chainId];
  ...
  const getOutputCurrency = () => {
    if (outputCurrency1 && outputCurrency1?.symbol === CICZ[chainId].symbol) {
      return CICZ[chainId];
