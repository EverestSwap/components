import { CICZ, ChainId, Currency, Token, currencyEquals } from '@everestswap/sdk';
...
const currencyKey = (currency: Currency, chainId: ChainId): string => {
  return currency instanceof Token ? currency.address : currency === CICZ[chainId] ? 'ICZ' : '';
};
...
const SelectTokenDrawer: React.FC<Props> = (props) => {
  ...
  const currencies = useMemo(() => [CICZ[chainId], ...filteredSortedTokens], [filteredSortedTokens, chainId]);
  //const currencies = useMemo(() => [Currency.CICZ, ...filteredSortedTokens], [filteredSortedTokens]);
