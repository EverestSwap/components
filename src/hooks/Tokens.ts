...
import { CICZ, Currency, Token } from '@everestswap/sdk';
...
export function useCurrency(currencyId: string | undefined): Currency | null | undefined {
  const chainId = useChainId();
  const isICZ = currencyId?.toUpperCase() === 'ICZ';
  const token = useToken(isICZ ? undefined : currencyId);
  return isICZ ? chainId && CICZ[chainId] : token;
}
