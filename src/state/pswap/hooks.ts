  ...
  const onCurrencySelection = useCallback(
    (field: Field, currency: Currency) => {
      dispatch(
        selectCurrency({
          field,
          currencyId: currency instanceof Token ? currency.address : currency === CICZ[chainId] ? 'ICZ' : '',
        }),
      );
    },
    [dispatch],
...
const BAD_RECIPIENT_ADDRESSES: string[] = [
  FACTORY_ADDRESS[ChainId.ICE_MAINNET], // v2 factory
  ROUTER_ADDRESS[ChainId.ICE_MAINNET], // v2 router 02
];
...
function parseCurrencyFromURLParameter(urlParam: any): string {
  if (typeof urlParam === 'string') {
    ...
    if (urlParam.toUpperCase() === 'ICZ') return 'ICZ';
    if (valid === false) return 'ICZ';
  }
  //return 'ICZ' ?? '';
  return '';
}
...
export function useGelatoLimitOrderDetail(order: Order) {
  ...
  const inputCurrency = order.inputToken === NATIVE && chainId ? 'ICZ' : order.inputToken;
  const outputCurrency = order.outputToken === NATIVE && chainId ? 'ICZ' : order.outputToken;
