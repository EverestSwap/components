import { CICZ, ChainId, Currency, CurrencyAmount, JSBI, Token, TokenAmount } from '@everestswap/sdk';
...
export function useCurrencyBalances(
  ...
  const containsETH: boolean = useMemo(
    () => currencies?.some((currency) => chainId && currency === CICZ[chainId]) ?? false,
    [chainId, currencies],
  );
  ...
  return useMemo(
    () =>
      currencies?.map((currency) => {
        ...
        if (currency === CICZ[chainId]) return ethBalance[account];
        return undefined;
      }) ?? [],
